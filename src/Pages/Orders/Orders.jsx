            ...
            {data.data.map((order) => (
              <div key={order._id} className='shadow-xl mt-6 p-5 border border-teal-400 rounded-md'>
                <div className='flex mb-5 items-center justify-between'>
                  <div><span className=' font-semibold text-lg'>Order ID : </span># {order.id}</div>
                  <div>
                    {order.isPaid ? (
                ...
                <div className='grid grid-cols-12'>
                  {order.cartItems.map((product) => (
                    <div key={product._id} className="product shadow-xl m-3 p-2 border border-teal-400 rounded-md col-span-2">
